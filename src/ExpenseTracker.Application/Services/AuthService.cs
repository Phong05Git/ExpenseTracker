...
IJwtTokenSettings jwtTokenSettings) : IAuthService
{
    ...
    CancellationToken cancellationToken = default)
{
    ...
    if (await userRepository.GetByUsernameAsync(username, cancellationToken) != null)
    {
        return Result<AuthResponseDto>.Failure("Username already exists.");
    }

    if (await userRepository.GetByEmailAsync(email, cancellationToken) != null)
    {
    ...
    CancellationToken cancellationToken = default)
{
    ...
    {
        return Result<AuthResponseDto>.Failure("Account is temporarily locked.");
    }
    ...
    if (!passwordHasher.Verify(request.Password, user.PasswordHash))
    {
        ...
        {
            user.Lock(DateTime.UtcNow.Add(LockDuration));
        }
...
public async Task<Result<AuthResponseDto>> RefreshTokenAsync(
    ...
    CancellationToken cancellationToken = default)
{
    var oldRefreshToken = await refreshTokenRepository.GetByTokenAsync(
    ...
    CancellationToken cancellationToken)
{
    var accessTokenExpiresAt =
        DateTime.UtcNow.AddMinutes(
            jwtTokenSettings.AccessTokenMinutes);

    var refreshTokenExpiresAt =
        DateTime.UtcNow.AddDays(
            jwtTokenSettings.RefreshTokenDays);
...
private void BlacklistAccessToken(string? accessToken)
{
    ...
    var expiresAt = tokenService.GetTokenExpiration(accessToken);

    if (!string.IsNullOrWhiteSpace(tokenId) && expiresAt.HasValue)
    {
        tokenBlacklistService.Revoke(tokenId, expiresAt.Value);
    }
}
